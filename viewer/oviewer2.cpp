...
void Keyboard(unsigned char key, int x, int y) {
  switch (key) {
    case 'Y': {
      OctViewer2* gvd_viewer = (OctViewer2*)scene.get();
...
int main(int argc, char** argv) {
  ...
  const float r = window_width / static_cast<float>(window_height);
  // scene.reset(new OctViewer2(int2(window_width, window_height),
  //                         float2(-r, -1), float2(r, 1)));
  scene.reset(new OctViewer2(window_width, window_height)),
