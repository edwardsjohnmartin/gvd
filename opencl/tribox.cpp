...
/********************************************************/
// File modified slightly by John Edwards 1/21/13
...
//                    // const std::vector<int3>& vertices,
//                    const int3* vertices,
//                    const Triangle& tri) {
bool TriBoxOverlap(const int3 boxcenter,
                   const int boxhalfsize,
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      // triverts[i][j] = vertices[tri.s[i]].s[j];
      triverts[i][j] = intn_comp(j, vertices[tri.s[i]]);
    }
  }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    // boxcenter_f[i] = (float)(boxcenter.s[i]);
    boxcenter_f[i] = (float)intn_comp(i, boxcenter);
  }
  const float boxhalfsize_v[] = { boxhalfsize_f, boxhalfsize_f, boxhalfsize_f };
  // return static_cast<bool>(triBoxOverlap(
  //     boxcenter_f,
  //     make_float3(boxhalfsize_f, boxhalfsize_f, boxhalfsize_f).s,
  //     triverts));
  return (bool)(triBoxOverlap(
      boxcenter_f,
      boxhalfsize_v,
      triverts));
